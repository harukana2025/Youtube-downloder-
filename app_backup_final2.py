...
document.getElementById('infoBtn').onclick=async function(){
    ...
    try{
        ...
        if(data.success){
            ...
            qs.innerHTML='';
            for(let f of formats){const opt=document.createElement('option');opt.value=f.itag;opt.textContent=f.height+'p'+(f.fps?f.fps+'fps':'');qs.appendChild(opt);}
            let def=formats.find(f=>f.height===720)||formats[0];
            if(def){selectedItag=def.itag;qs.value=selectedItag;document.getElementById('videoSource').src=def.preview_url;document.getElementById('videoPreview').load();}
...
document.getElementById('qualitySelect').onchange=function(){
    ...
    if(fmt){selectedItag=itag;document.getElementById('videoSource').src=fmt.preview_url;document.getElementById('videoPreview').load();}
};
...
LIVE_HTML = NORMAL_HTML  # 実際は元のLIVE_HTMLを使用
